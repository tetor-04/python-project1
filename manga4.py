...
class RibbonApp:
    ...
    def preview_selected(self, tree):
        ...
        if os.path.isfile(fullpath) and ext in ('.jpg', '.jpeg', '.png'):
            try:
    ...
    def merge_manga_structure(self, src, dest):
        for root, dirs, files in os.walk(src):
            if any(os.path.splitext(f)[1].lower() in ['.jpg', '.jpeg', '.png'] for f in files):
    ...
    def merge_or_copy_images(self, src_folder, dest_folder):
        images = sorted(
            [f for f in os.listdir(src_folder) if os.path.splitext(f)[1].lower() in ['.jpg', '.jpeg', '.png']],
        ...
        for i in range(0, num_images, group_size):
            ...
            for name in group:
                try:
                    img = Image.open(os.path.join(src_folder, name)).convert("RGBA")
                    imgs.append(img)
                except Exception as e:
    ...
    def get_text_font_size(self):
        try:
            ...
            return tkfont.Font(font=font).cget("size")
        except Exception:
            return 12
