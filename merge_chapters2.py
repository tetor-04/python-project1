...
class RibbonApp:
    ...
    def create_manga_tab(self):
        manga_controls = ttk.Frame(self.manga_tab, padding=10)
        manga_controls.pack(fill='x')

        ttk.Button(manga_controls, text='Select Source Folder', command=self.select_manga_folder).pack(side='left', padx=5)
        ttk.Button(manga_controls, text='Select Output Folder', command=self.select_output_folder).pack(side='left', padx=5)
        ttk.Button(manga_controls, text='Merge', command=self.merge_selected_manga).pack(side='left', padx=5)
    ...
    def merge_selected_manga(self):
        ...
        popup.title("Merging Manga")
        popup.geometry("550x450")
        popup.grab_set()

        ttk.Label(popup, text="Merging in progress...").pack(pady=5)
        progress = ttk.Progressbar(popup, orient="horizontal", length=500, mode="determinate")
        progress.pack(pady=5)
        status_label = ttk.Label(popup, text="Starting...", anchor='w', justify='left')
        status_label.pack(fill='x', padx=5)
        image_label = ttk.Label(popup)
        image_label.pack(pady=10)
        log_text = tk.Text(popup, height=12, wrap="word", state='normal')
        log_text.pack(fill='both', expand=True, padx=5, pady=5)

        chapters = [f for f in os.listdir(self.manga_folder) if os.path.isdir(os.path.join(self.manga_folder, f))]
        total_chapters = len(chapters)
        progress["maximum"] = total_chapters

        def merge_all():
            for i, manga_name in enumerate(chapters, 1):
                manga_path = os.path.join(self.manga_folder, manga_name)
                output_path = os.path.join(self.output_folder, manga_name)
                status_label.config(text=f"Merging Chapter: {manga_name}")
                self.root.update()
                self.merge_manga_structure(manga_path, output_path, log_text, image_label)
                progress["value"] = i
                self.root.update()
        ...
        self.root.after(100, merge_all)

    def merge_manga_structure(self, src, dest, log_widget, image_label):
        for root, dirs, files in os.walk(src):
            if any(f.lower().endswith(('.jpg', '.png', '.jpeg')) for f in files):
                rel_path = os.path.relpath(root, src)
                target_path = os.path.join(dest, rel_path)
                self.merge_or_copy_images(root, target_path, log_widget, image_label)

    def merge_or_copy_images(self, src_folder, dest_folder, log_widget, image_label):
        images = sorted(
        ...
        if num_images <= 25:
            for img in images:
                ...
                dst = os.path.join(dest_folder, os.path.splitext(img)[0] + ".webp")
                with Image.open(src) as im:
                    im.convert("RGB").save(dst, "WEBP")
            return
        ...
        for i in range(0, num_images, group_size):
            ...
            log_entry = f"Merging {group[0]} ... {group[-1]} → {index:03}.webp\n"
            ...
            final_path = os.path.join(dest_folder, f"{index:03}.webp")
            merged.convert("RGB").save(final_path, "WEBP")
    ...
    def copy(self):  # Clipboard / Edit Features
        try:
